...
class FitnessAgent:
    ...
    def _format_physical_data(self, d: dict) -> str:
        ...
        if preferred_group and preferred_group != 'сбалансированно':
            # Конкретизируем рекомендации по упражнениям для каждой группы
            exercises_hint = {
                'ноги': 'приседания (классические, фронтальные, болгарские), выпады, жим ногами, разгибания/сгибания ног, гакк-приседания',
                'ягодицы': 'ягодичный мостик, отведения в тренажёре, румынская тяга, выпады назад, болгарские сплит-приседы, гиперэкстензии с акцентом на ягодицы',
                'спина': 'становая тяга, тяги штанги/гантелей в наклоне, подтягивания, тяги верхнего/горизонтального блока разными хватами, тяга Т-грифа, шраги',
                'плечи и руки': 'жимы штанги/гантелей стоя и сидя, махи гантелей (в стороны, вперёд, в наклоне), тяги к подбородку, подъёмы штанги/гантелей на бицепс, французский жим, отжимания на брусьях'
            }
            
            hint = exercises_hint.get(preferred_group, '')
            result += (
                f"\n\n🎯 ГЛАВНЫЙ ПРИОРИТЕТ ПРОГРАММЫ:\n"
                f"Пользователь хочет сделать МАКСИМАЛЬНЫЙ АКЦЕНТ НА {preferred_group.upper()}!\n"
                f"ОБЯЗАТЕЛЬНО:\n"
                f"- Включи минимум 60-70% упражнений на {preferred_group} в каждую тренировку\n"
                f"- Начинай каждую тренировку с базовых упражнений на {preferred_group}\n"
                f"- Используй разнообразные упражнения: {hint}\n"
                f"- Добавь 1-2 изолирующих упражнения дополнительно на {preferred_group}\n"
                f"- Другие группы мышц — второстепенны, давай им меньше объёма"
            )
