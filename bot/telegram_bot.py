...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not completed and state.get("mode") == "survey":
        ...
        await update.message.reply_text("Выбери свой уровень подготовки:", reply_markup=ReplyKeyboardMarkup(
            [LEVEL_CHOICES], resize_keyboard=True, one_time_keyboard=True
        ))
        return

    if state.get("mode") == "awaiting_level":
        if text not in LEVEL_CHOICES:
            await update.message.reply_text("Пожалуйста, выбери уровень кнопкой ниже:",
...
async def on_program_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "new":
        ...
        try:
            ...
            await q.message.reply_text("Не получилось сгенерировать новую программу. Попробуй ещё раз.")
            return
        await q.message.reply_text("Готово! Держи альтернативный вариант ⬇️")
        await _send_program(update, user_id, plan)
        return
    ...
    if action == "restart":
        ...
        user_data["current_plan"] = ""
        save_user_data(user_id, user_data)
        user_states[user_id] = {"mode": "awaiting_name", "step": 0, "data": {}} if not name else {"mode": None, "step": 0, "data": {}}
        if name:
